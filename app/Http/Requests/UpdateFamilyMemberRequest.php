...
class UpdateFamilyMemberRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'dod' => 'sometimes|nullable|date|after:dob',
            'biodata' => 'sometimes|nullable|string|max:1000',
            'profile_pic' => 'sometimes|nullable|string|max:1000',
            'position_x' => 'sometimes|nullable|numeric',
            'position_y' => 'sometimes|nullable|numeric',
        ];
    }
    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            ...
            if ($this->has('is_alive') && $this->input('is_alive') === false && empty($this->input('dod'))) {
                $validator->errors()->add('dod', 'Date of death is required when marking someone as deceased.');
            }
            
            if ($this->has('is_alive') && $this->input('is_alive') === true && !empty($this->input('dod'))) {
